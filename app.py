...
def find_superslicer():
    """Find SuperSlicer executable with correct paths"""
    ...
    # Check paths based on environment
    candidates = render_candidates if (os.environ.get('RENDER') or os.path.exists('/opt/render')) else local_candidates
...
def extract_actual_gcode_time(gcode_path):
    """
    Extract actual print time from SuperSlicer generated G-code comments
    Returns (time_string, time_in_seconds) or (None, None) if not found
    """
    ...
    time_patterns = [
        # "; estimated printing time (normal mode) = 2h 34m 56s"
        r";\s*estimated printing time.*?=\s*(\d+)h\s*(\d+)m\s*(\d+)s",
        # "; estimated printing time: 2h 34m 56s"
        r";\s*estimated printing time.*?:\s*(\d+)h\s*(\d+)m\s*(\d+)s",
        # "; Print time: 2h 34m 56s"
        r";\s*print time.*?:\s*(\d+)h\s*(\d+)m\s*(\d+)s",
        # "; TIME:7856" (seconds)
        r";\s*TIME:\s*(\d+)",
        # "; Total print time: 154m 23s"
...
def index():
    if request.method == "POST":
        ...
        if not SUPERSLICER_PATH:
            ...
            print(f"🚀 Running SuperSlicer command: {' '.join(cmd)}")
            ...
            try:
                ...
                if os.path.exists(output_gcode) and os.path.getsize(output_gcode) > 100:
                    ...
                    print_time, time_seconds = extract_actual_gcode_time(output_gcode)
                    ...
                    if print_time and time_seconds:
                        ...
                        # Fallback to demo calculation if time extraction fails
                        print_time, cost = calculate_demo_estimate(infill, wall_thickness, filename)
                        is_demo = True
                        flash("G-code generated but time extraction failed. Showing estimated values.", "warning")
                        print("⚠️  Using demo calculation as fallback")
                else:
